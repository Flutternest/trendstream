...
final paginatedPopularMoviesProvider =
  ...
  (ref, int pageIndex) async {
    ...
    if (selectedGenre?.id == 0 || selectedGenre == null) {
      ...
      filteredMovies = dummyMovies.where((movie) => 
        movie.genres?.any((genre) => genre.id == selectedGenre.id) ?? false
      ).toList();
    }
    ...
    final int endIndex = (startIndex + itemsPerPage).clamp(0, filteredMovies.length);
    
    final List<Movie> pageResults = filteredMovies.sublist(
      startIndex, 
      endIndex
    );
